 ...
 * supported.
 */
class ConcurrentVersionsTree[A, B](private val root: ConcurrentVersionsTree.Node[A]) extends ConcurrentVersions[A, B] {
  ...
  private def copy(): ConcurrentVersionsTree[A, B] =
    new ConcurrentVersionsTree[A, B](root.copy()).withOwner(_owner).withProjection(_projection)

  private def nodes: Seq[Node[A]] = foldLeft(root, Vector.empty[Node[A]]) {
    case (acc, n) => acc :+ n
  }

  private def leaves: Seq[Node[A]] = foldLeft(root, Vector.empty[Node[A]]) {
    case (leaves, n) => if (n.leaf) leaves :+ n else leaves
  }

  private def pred(timestamp: VectorTime): Node[A] = foldLeft(root, root) {
  ...
  private def foldLeft[C](node: Node[A], acc: C)(f: (C, Node[A]) => C): C = {
    val acc2 = f(acc, node)
    node.children match {
      case Seq() => acc2
...
object ConcurrentVersionsTree {
  ...
  private class Node[A](var versioned: Versioned[A]) extends Serializable {
